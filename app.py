...
if active_screen == "Pricing":
    ...
    with p1:
        st.markdown("### Starter")
        st.write("$0 / month")
        st.write("1 active comparison table")
        st.write("Basic filters")
    with p2:
        st.markdown("### Pro")
        st.write("$19 / month")
        st.write("Unlimited comparisons")
        st.write("AI best-match ranking")
    with p3:
        st.markdown("### Team")
        st.write("$49 / month")
        st.write("Shared workspaces")
        st.write("Priority support")

    st.caption("Contact support for plan customization.")
    st.stop()
...
with right_col:
    ...
    if parsed_listing:
        ...
        st.write(
            ...
            + (f" ({walkability.get('walk_label')})" if walkability.get('walk_label') else "")
        )
        st.write(
            f"**Transit Score:** "
            f"{walkability.get('transit_score') if walkability.get('transit_score') is not None else 'N/A'}"
            + (f" ({walkability.get('transit_label')})" if walkability.get('transit_label') else "")
        )
        st.write(
            f"**Bike Score:** "
            f"{walkability.get('bike_score') if walkability.get('bike_score') is not None else 'N/A'}"
            + (f" ({walkability.get('bike_label')})" if walkability.get('bike_label') else "")
        )
