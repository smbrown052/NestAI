...
def _fresh_state(**kwargs):
    """Reset the shared session_state in-place and return it.

    Updating in-place (rather than creating a new object) preserves the
    reference held by other test modules that imported streamlit before
    or after this module was loaded, preventing cross-test state leaks.
    """
    defaults = {
        "nestai_tier": "free",
...
class TestCanEnrichBuilding:
    def test_premium_plus_can_always_enrich(self):
        _fresh_state(nestai_tier="premium_plus", nestai_analyses_used=500)
    ...
    def test_free_tier_blocked_at_limit(self):
        _fresh_state(nestai_tier="free", nestai_analyses_used=5)
        from credits import can_enrich_building
...
class TestConsumeAnalysis:
    def test_premium_plus_consumes_without_limit(self):
        _fresh_state(nestai_tier="premium_plus", nestai_analyses_used=999)
    ...
    def test_free_tier_succeeds_and_increments(self):
        state = _fresh_state(nestai_tier="free", nestai_analyses_used=0)
        from credits import consume_analysis
        result = consume_analysis("bldg_ok")
        assert result is True
        assert state.nestai_analyses_used == 1

    def test_idempotent_same_building(self):
        state = _fresh_state(nestai_tier="free", nestai_analyses_used=4)
        from credits import consume_analysis
        consume_analysis("bldg_repeat")
        count_after_first = state.nestai_analyses_used
        consume_analysis("bldg_repeat")
        assert state.nestai_analyses_used == count_after_first  # no double charge
