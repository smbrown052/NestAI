...
if active_screen == "Login":
    st.markdown("### 🔐 Login")
    username = st.text_input("Email or Username", key="screen_login_username")
    password = st.text_input("Password", type="password", key="screen_login_password")
    remember_me = st.checkbox("Remember me", key="screen_login_remember")

    if st.button("Sign In", key="screen_login_submit"):
        if username and password:
            st.success("Login submitted.")
            if remember_me:
                st.caption("Your sign-in preference was saved for this session.")
        else:
