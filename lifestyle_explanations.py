...
def generate_lifestyle_explanation(
    ...
    if factor1_name == "commute":
        ...
        explanation_parts.append(f"   • {factor1_score:.0f}/100: Just {metro_min} min from metro")
        ...
        nearby_venues = row.get("nearby_restaurants", 0) + row.get("nearby_bars", 0)
    ...
    if len(top_factors) > 1:
        ...
        if factor2_name == "commute":
            metro_min = row.get("metro_min")
            explanation_parts.append(f"   • {factor2_score:.0f}/100: {metro_min} min to transit")
